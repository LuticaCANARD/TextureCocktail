...
namespace LuticaLab.TextureCocktail
{
    ...
    public class ImageViewerWindow : EditorWindow
    {
        ...
        public static void ShowWindow(TextureCocktail source, string name = "Preview")
        {
            var window = GetWindow<ImageViewerWindow>(name);
            window.BindSource(source);
            window.SetRenderTexture(source != null ? source.PreviewTexture : null);
            window.Show();
        }
        ...
        private void BindSource(TextureCocktail source)
        {
            ...
            if (_source != null)
            {
                _source.OnPreviewUpdated += Repaint;
            }
        }
        ...
        private void OnDestroy() { UnbindSource(); }

        private void OnGUI()
        {
            bool sourceAlive = _source != null;
            // If source was destroyed externally, fall back gracefully.
            if (sourceAlive && _source.PreviewTexture != null)
            {
                _renderTexture = _source.PreviewTexture;
            }
            ...
            // Polygon control row (only when bound to a TextureCocktail source)
            if (sourceAlive)
            {
                EditorGUILayout.BeginHorizontal(EditorStyles.toolbar);
            ...
            if (_fitToWindow)
            {
                ...
                if (sourceAlive)
                {
                    _source.DrawPolygonMaskOverlay(centeredRect);
                    EventType beforeType = Event.current.type;
                    _source.HandlePolygonMaskInteraction(centeredRect, Event.current);
                    if (Event.current.type == EventType.Used && beforeType != EventType.Used)
                    {
                        _source.Repaint();
                    }
                }
            ...
            else
            {
                ...
                Rect contentRect = new Rect(0, 0, displayTexture.width * _zoom, displayTexture.height * _zoom);
                GUI.DrawTexture(contentRect, displayTexture, ScaleMode.StretchToFill);

                if (sourceAlive)
                {
                    _source.DrawPolygonMaskOverlay(contentRect);
                    EventType beforeType = Event.current.type;
                    _source.HandlePolygonMaskInteraction(contentRect, Event.current);
                    if (Event.current.type == EventType.Used && beforeType != EventType.Used)
                    {
                        _source.Repaint();
                    }
